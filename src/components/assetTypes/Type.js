...
const Type = () => {
  ...
  const handleRowAdd = (newData, resolve) => {
    let errorList = [];
    if (newData.asset_type === undefined) {
  ...
  const handleRowUpdate = (newData, oldData, resolve) => {
    ...
    //  console.log(newData);
    let errorList = [];
    if (newData.asset_type === "") {
      //alert("oooo");
      errorList.push("Asset type can't be empty   ");
      setIserror(true);
    }

    if (errorList.length < 1) {
      axios
        .post(`${URD}/type/update/${oldData.id}`, newData)
        .then((response) => {
